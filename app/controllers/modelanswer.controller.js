const Modelanswer = require("../models/modelanswer.model.js");
console.log("300");
...
exports.create = (req, res) => {
 ...
 Modelanswer.create(modelanswer, (err, data) => {
    if (err)
      res.status(500).send({
        message:
...
exports.findAll = (req, res) => {
  ...
  console.log("400");
 Modelanswer.getAll(title_code, (err, data) => {
    if (err)
      res.status(500).send({
        message:
...
exports.findOne = (req, res) => {
 Modelanswer.findById(req.params.id, (err, data) => {
...
exports.findAllPublished = (req, res) => {
 ...
 Modelanswer.getAllPublished((err, data) => {
    if (err)
      res.status(500).send({
        message:
          err.message || "Some error occurred while retrieving tutorials."
 ...
 Modelanswer.updateById(
    ...
    (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          res.status(404).send({
            message: `Not found model answer with id ${req.params.id}.`
          });
        } else {
          res.status(500).send({
            message: "Error updating model answer with id " + req.params.id
          });
        }
      } else res.send(data);
    }
...
exports.delete = (req, res) => {
 Modelanswer.remove(req.params.id, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Not foundmaster with id ${req.params.id}.`
...
exports.deleteAll = (req, res) => {
 Modelanswer.removeAll((err, data) => {
    if (err)
      res.status(500).send({
        message:
